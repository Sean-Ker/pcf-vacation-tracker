...
import { IdentityContext, CountriesContext } from "../Contexts";
import LoadingScreen from "./LoadingScreen";

const Register = () => {
    const { user, setUser } = useContext(IdentityContext);
    const { allLocations, setAllLocations } = useContext(CountriesContext);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const userRes = await axios.get("/users/uuid/" + uuid).catch(res => {
                debugger;
    ...
    if (loading > 0) return <LoadingScreen />;
